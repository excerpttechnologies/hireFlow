...
import { Sun, Moon, Menu, X, Bell, ChevronDown, Briefcase, Building2, LayoutDashboard } from 'lucide-react'
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="flex items-center gap-2">
            ...
            <div className="hidden sm:flex items-center gap-2">
                ...
                <AnimatePresence>
                  {dropdownOpen && (
                    <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 6 }}
                      ...
                      <Link to="/login/student" onClick={() => setDropdownOpen(false)}
                        ...
                        <div>
                          <p className="text-sm font-medium text-gray-900 dark:text-white">Job Seeker</p>
                          <p className="text-xs text-gray-400">Find your dream job</p>
                        </div>
                      </Link>
                      <Link to="/login/employer" onClick={() => setDropdownOpen(false)}
                        className="flex items-center gap-3 px-4 py-3 hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors">
              ...
              <Link to="/register/student" className="btn-primary text-sm py-2 px-4 rounded-xl">
                Get Started
              </Link>
            </div>
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <div className="px-4 py-3 space-y-1">
              ...
              <div className="pt-2 border-t border-gray-100 dark:border-gray-800 flex gap-2">
                <Link to="/login/student" onClick={() => setMenuOpen(false)} className="flex-1 btn-secondary text-sm text-center py-2 rounded-xl">Login</Link>
                <Link to="/register/student" onClick={() => setMenuOpen(false)} className="flex-1 btn-primary text-sm text-center py-2 rounded-xl">Register</Link>
              </div>
            </div>
          </motion.div>
        )}
